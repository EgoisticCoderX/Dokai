...
# Initialize httpx client with proxies disabled
http_client = httpx.Client(proxies=None)

# Initialize Groq client with the custom http_client
client = groq.Client(api_key=api_key, http_client=http_client)

def generate_response(prompt, web_search_enabled=False, model="llama-3.3-70b-versatile", max_tokens=500):
    """
...
if __name__ == "__main__":
    ...
    while True:
        ...
        web_search_needed = (choice == '2')
        
        response = generate_response(user_input, web_search_needed=web_search_needed)
        print(f"\nDokai: {response}")